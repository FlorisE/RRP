        ...
        constructor(streamModule, id, name, x, y, program) {
            ...
            this.in = ko.computed(function () {
                return self.program.operations().filter(
                    (operator) => operator.destination() == self.id()
                );
            }, this);
            ...
            this.out = ko.computed(function () {
                return self.program.operations().filter(
                    (operator) => {
                        if (operator["source"]) {
                            return operator.source() == self.id();
                        } else if (operator["sources"]) {
                            return operator.sources().find(
            ...
            this.deleteClicked = function(operator) {
