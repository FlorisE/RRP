...
OPTIONAL MATCH (stream)-[:actuator]->(actuator:Actuator)
RETURN stream, program, draw, directInStream, collect(indirectInStream) as indirectInStreams, sensor, actuator`,
      {id: id}
    ).then(
  ...
  mapStream(item) {
    return {
      ...
      parameters: item.parameters ?
        item.parameters.map(this.mapParameter) :
        null,
  ...
  mapParameter(parameter) {
    ...
    if (parameter.value) {
      if (parameter.type === "integer") {
        returnValue.value = parameter.value["low"] !== undefined ?
          parameter.value.low :
          parameter.value;
      } else if (parameter.type === "select") {
        returnValue.value = parameter.value;
      } else {
        returnValue.value = parameter.value;
      }
    }
    ...
    if (parameter.type === "select") {
      returnValue.options = parameter.options ?
